...
`loadmill/lloyd-qa-action` runs one Lloyd-selected [Droid CUA](https://www.npmjs.com/package/@loadmill/droid-cua) test against an Android APK and reports structured progress and results to Loadmill.

The Action is deliberately a thin wrapper. It installs the pinned `@loadmill/droid-cua@2.29.0` package in the runner's temporary directory before checking out PR code, then invokes its existing CLI flags. Lloyd-specific job state remains inside this Action; Droid CUA has no Lloyd-specific flag or behavior.
...
on:
  workflow_dispatch:
    inputs:
      ...
      test_path: {required: true, type: string}
...
| `test_path` | Yes | Repository-relative path to one `.dcua` test. |
...
Absolute paths, traversal outside the checkout, symlink escapes, missing files, and artifacts containing zero or multiple APKs are rejected.
...
- OS: Android 14
- Droid timeout: 25 minutes
- Video artifacts, HTML report, and debug logging enabled

The working directory is the checked-out repository root. Droid's `logs` and `droid-cua-artifacts` directories, the HTML report, and the combined runner log are collected when present.
...
Progress callback failures are warnings and do not interrupt the test. Failure to deliver the final completion callback fails the Action.
...
The Action succeeds only for `passed`. Test, infrastructure, cancellation, results-upload, and completion-delivery failures return a failing Action status.
